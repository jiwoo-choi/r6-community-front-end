...
export class R6List extends React.PureComponent<ForumReactorProps>{
    ...
    public getList( cellData : ListType[]) {
        ...
        for (let i = 0 ; i < cellData.length ; i++) {
            cell.push( 
                <R6Cell onClick={ this.dispatcher({type:"CLICKPOST", postId: cellData[i].postId}) } isNotice={cellData[i].notice} key={"CELL_"+i} data={cellData[i]}/>
            )
        }
        return cell
    }
...
export default withReactor(R6List, (state) => ({isLoading: state.isLoading, list:state.list, state:state.isError}));
